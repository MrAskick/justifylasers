    ...
    private static final class Serializer implements RecipeSerializer<IndustryRecipe> {
        @Override public IndustryRecipe read(Identifier id, JsonObject json) {
            ...
            return new IndustryRecipe(id, new MachineRecipeData(JsonHelper.getString(json, "key"), MachineKind.valueOf(JsonHelper.getString(json, "machine")),
                    ...
                    json.has("process") ? RecipeProcess.CODEC.parse(com.mojang.serialization.JsonOps.INSTANCE, json.get("process"))
                            .getOrThrow(false, message -> { throw new IllegalArgumentException(message); }) : RecipeProcess.BASIC));
        }
        @Override public IndustryRecipe read(Identifier id, PacketByteBuf buffer) {
            ...
            return new IndustryRecipe(id, new MachineRecipeData(key, kind, blueprint, ingredients, counts, buffer.readItemStack(), buffer.readVarInt(), buffer.readVarInt(), buffer.readVarInt(), RecipeProcess.read(buffer)));
        }
