...
An LM-powered emitter projects a walkable, translucent surface without placing support blocks or spawning entities. Aim a powered laser or solar beam at any face except its front aperture. Mount emitters on floors, walls or ceilings and aim in any of six directions. Up to three adjacent, aligned straight emitters form a continuous 1-, 2- or 3-wide field. Corner emitters join floor, wall and ceiling sections into hard-light tunnels; each corner costs two lanes of the shared length budget. Connected sections share incoming light and color. The Configurator's Ports mode changes the output direction; its 45° mode rolls the surface around that direction, including upward/downward projections. Diagonal surfaces have matching thin collision, not a solid bounding box.

Hold **Left Ctrl** (rebindable) with the Configurator, point at a radial-menu sector and release to select. Its indicator follows the selected mode. The tool stores **20,000 FE** and spends **20 FE per successful configuration**, with free creative use. Charge it in a generator, a compatible energy charger, or from a powered machine using Charge mode. Existing tools start uncharged; saved modes and copied settings remain intact.

Length costs **2,000 lm per block of length per emitter section**, capped at **256 blocks** by default (`lightBridgeMaxLength`, 1–512). Wider bridges need proportionally more light. The low-profile emitter sits flush against its mounting face and the field reaches adjoining obstacles without a recessed gap; the surface is inset 0.1 blocks from each outer side, with matching collision. Real collision shapes, unloaded server chunks and the world border stop the projection. Removing power removes both the light and its physical support. Lithium is supported without disabling its movement optimizations. The emitter has an Assembly Chamber recipe and a tablet schematic. See the [Light Bridge guide (Russian)](docs/light-bridge.ru.md) for materials, power examples and configuration.

Healing is fixed at **0.1 HP per second**, independent of old sliders. All module LM prices are doubled; installed and placed Mining use the same optical cost and paid work speed. Inventory ports use Fabric Transfer API or Forge/NeoForge item capabilities; private inventories and slot restrictions still apply. Beam attenuation is optional and disabled by default (`beamAttenuation`, `beamLossPerBlock`).

## Beam modules

The powered emitter's Effect slot accepts **Mining, Damage, Heal, Lift or Lower**. Its entity filter supports searchable registry-based mob selection, including modded creatures, with blacklists/whitelists; the turret uses the same selector. Mining contains Collection, Silk Touch, Drops and Ignition upgrade slots, a mining toggle and a speed control. Ignition smelts mined drops using furnace recipes; in Damage settings it ignites targets instead. Collected loot goes into a nine-slot inventory; overflow drops into the world. Scorch marks remain available, but new Scorch module insertion is disabled.

Place modules on a beam's path and right-click them to configure downstream effects. **Effects combine**: Lift with Heal raises and heals, while equal Lift and Lower cancel. Solar concentrators remain module-free: upgrade their rays with placed modules. Installed effects consume LM as well as the emitter's FE draw; placed modules consume incoming LM. A combined incident beam pays a module once, regardless of the number of contributing sources. Creative emitters supply adjustable 0–1 Tlm test power. Schematics have a separate creative tab; raw photonite and wolframite are placeable decorations.

See the [beam-module guide (Russian)](docs/beam-modules.ru.md) for effect composition, costs, filtering, storage and migration of existing inventories.

## Beam Combiner and recipe navigation

The **Beam Combiner** merges up to five incident beams into one output, retaining **95% of their total luminous flux** per combiner. The Energy Receiver converts that combined flux with its usual efficiency. The Configurator cycles each face through input, output and disabled; selecting another output returns the previous output to input. Combiners can touch directly, including vertical chains, without an air gap. Incoming range is not renewed, and loops cannot repeat a source's energy budget. Output color is flux-weighted; mirrors, splitters, crystals and refocusing cubes remain compatible. `beamCombinerEfficiency` controls the loss (default `0.95`). The combiner requires its Assembly Chamber schematic.
...
The new **Small Solar Concentrator** fits one block, tracks the sun and supplies up to **16 klm**, with a 32-block beam. Its tablet schematic assembles three Solar Absorbers, one Electric Motor and three Iron Ingots, costing 56,320 FE over 44 working seconds. The motor has a detailed, placeable 3D model and uses a workbench recipe: `ICI / CRC / ICI` (iron, copper, redstone). Each absorber still costs 26,400 FE to assemble.

**Crystal Growth Chambers consume only light**, not FE. Aim at either lower lens on any horizontal face. Growth speed scales directly with the summed input: **480 klm = 1×**, completing a batch in 600 working ticks (30 seconds at 20 TPS). At 240 klm it takes 60 seconds; at 960 klm, 15 seconds. A batch still uses raw photonite, two quartz and 1000 mB water. There is no minimum flux threshold: even weak sunlight makes fractional progress, which survives saving. No light pauses growth; excess light accelerates the current batch but is not stored. At most one batch completes per tick. The generator is not a substitute for an optical input.
...
| Crystal Growth Chamber | Raw Photonite + 2 quartz + 1,000 mB water → Photonite Crystal | 30 s at 480 klm; speed scales with input; no FE |
...
Other mods must implement the platform's energy interface for power transfer; no direct Mekanism dependency is required. Alpha.28 migrates stock bridge range (64 → 256) and growth reference flux (12,000 → 480,000), making `justifylasers.pre-alpha28.json.bak` first. Earlier stock generator/solar values are also migrated where needed; explicit nonstock values are retained. `crystalGrowthFlux` now means the input for 1× growth speed, not a minimum required input. Missing new keys use defaults.
...
The four original laser recordings and `saber_clash.ogg` remain directly under `assets/justifylasers/sounds/`. Resource packs can override the files and events in `sounds.json`. All shipped recordings are OGG Vorbis; normal builds need neither FFmpeg nor private MP3/WAV inputs. Local authoring tools are not published. The client menu's volume multiplier applies on top of `laserVolume`.
